...
{ 	
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;				 //NRF_CSN
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOA, &GPIO_InitStructure);                   //初始化指定IO,推挽 
	
	//初始化NRF_IRQ
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_8;   			 //NRF_IRQ
...
//	NRF24L01_Write_Reg(NRF_WRITE_REG+RX_PW_P0,RX_PLOAD_WIDTH);				//选择所有通道的有效数据宽度
	...
	{
		NRF24L01_Write_Reg(NRF_WRITE_REG+CONFIG, 0x0b);						//配置基本工作模式的参数;PWR_DOWN,EN_CRC,CRC_1byte,接收模式 
	}
	if( RxTx_mode == TX_MODE )
	{
		NRF24L01_Write_Reg(NRF_WRITE_REG+CONFIG, 0x0a);						//配置基本工作模式的参数;PWR_DOWN,EN_CRC,CRC_1byte,发送模式 
	}
